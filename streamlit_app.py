...
# --- استایل ۳ بعدی، رنگ کرمی-قهوه‌ای و آیکون‌ها ---
st.markdown(
    """
    <style>
    ...
    * { font-family: 'Vazirmatn', sans-serif !important; }
    ...
    .hero-3d-card {
        ...
        padding: 20px 24px;
        color: white;
        box-shadow: 0 15px 35px rgba(122, 75, 46, 0.25);
        display: flex;
        align-items: center;
    ...
    .hero-title { font-size: 1.5rem; font-weight: 900; margin: 0; color: #FFF8F0; }
    .hero-subtitle { font-size: 0.85rem; color: #EADFCF; margin-top: 5px; margin-bottom: 0; }
    .goat-avatar {
        width: 75px;
        height: 75px;
        border-radius: 50%;
    ...
    .stTabs [aria-selected="true"] {
        ...
        box-shadow: 0 4px 12px rgba(122, 75, 46, 0.15) !important;
    }

    /* کارت‌های آمار سه بعدی */
    ...
    div[data-testid="stMetric"] label { font-weight: bold; color: #7A6354; font-size: 0.9rem; }
    div[data-testid="stMetric"] div[data-testid="stMetricValue"] { color: #7A4B2E; font-size: 1.7rem; font-weight: 900; }
    ...
    .stButton > button {
        ...
        box-shadow: 0 6px 15px rgba(140, 90, 60, 0.25) !important;
        width: 100%;
    }
...
with tab_dash:
  ...
  total_milk_today = (
      conn.execute(
          "SELECT SUM(morning_kg + evening_kg) FROM milk_records WHERE"
          " record_date = ?",
          (datetime.today().strftime("%Y-%m-%d"),),
      ).fetchone()[0]
      or 0.0
  )
  active_pregnant = conn.execute(
      "SELECT COUNT(*) FROM reproduction"
  ).fetchone()[0]

  today_str = datetime.today().strftime("%Y-%m-%d")
  quarantine_count = conn.execute(
      "SELECT COUNT(DISTINCT tag_id) FROM treatments WHERE safe_date > ?",
      (today_str,),
  ).fetchone()[0]
  conn.close()
  ...
  if quarantine_count > 0:
    st.error(
        f"⚠️ **هشدار قرنطینه:** شیر {quarantine_count} رأس بز به دلیل دوره پرهیز"
        " دارویی نباید وارد تانک اصلی شود!"
    )
  else:
    st.success(
        "✨ **وضعیت بهداشتی گله عالی است:** تمام شیرهای تولیدی امروز قابل"
        " تحویل می‌باشند."
    )
...
with tab_goat:
  ...
  with st.form("add_goat_form", clear_on_submit=True):
    ...
    with col1:
      ...
      breed = st.selectbox(
          "نژاد بز:",
          ["سانن (Saanen)", "آلپاین (Alpine)", "مورسیا (Murcia)", "بومی / ترکیبی"],
      )
    with col2:
      birth_date = st.date_input("تاریخ تولد:")
      parity = st.number_input(
          "شکم زایش (Parity):", min_value=1, max_value=10, value=1
      )
  ...
  conn = get_db_connection()
  df_goats = pd.read_sql_query(
      "SELECT tag_id AS 'شماره پلاک', rfid_code AS 'کد RFID', breed AS 'نژاد',"
      " birth_date AS 'تاریخ تولد', parity AS 'شکم زایش' FROM goats",
      conn,
  )
  conn.close()
...
with tab_milk:
  ...
  goat_list = [
      row["tag_id"]
      for row in conn.execute("SELECT tag_id FROM goats").fetchall()
  ]
  conn.close()
    ...
    with st.form("milk_form", clear_on_submit=True):
      ...
      with col1:
        morning = st.number_input(
            "شیر دوشش صبح (kg):", min_value=0.0, step=0.1
        )
        fat = st.number_input(
            "درصد چربی (%):", min_value=0.0, max_value=10.0, value=3.5, step=0.1
        )
      with col2:
      ...
      if submit_milk:
        conn = get_db_connection()
        conn.execute(
            "INSERT INTO milk_records (tag_id, record_date, morning_kg,"
            " evening_kg, fat_pct, protein_pct) VALUES (?, ?, ?, ?, ?, ?)",
  ...
  if goat_list:
    with st.form("insemination_form", clear_on_submit=True):
      ...
      if submit_insem:
        ...
        conn = get_db_connection()
        conn.execute(
            "INSERT INTO reproduction (tag_id, insemination_date,"
            " expected_kidding_date, dry_off_date) VALUES (?, ?, ?, ?)",
            (
                selected_goat,
                str(insemination_date),
                str(expected_kidding),
                str(dry_off),
            ),
        )
        conn.commit()
        conn.close()
        st.success(
            f"تاریخ زایش تخمینی: {expected_kidding} | تاریخ خشک کردن: {dry_off}"
        )
...
with tab_health:
  ...
  if goat_list:
    with st.form("treatment_form", clear_on_submit=True):
      ...
      with col2:
        withdrawal_days = st.number_input(
            "روزهای پرهیز شیر:", min_value=1, max_value=30, value=5
        )
      ...
      if submit_treatment:
        ...
        conn = get_db_connection()
        conn.execute(
            "INSERT INTO treatments (tag_id, disease, drug, treatment_date,"
            " withdrawal_days, safe_date) VALUES (?, ?, ?, ?, ?, ?)",
            (
                selected_goat,
                disease,
                drug,
        ...
        conn.close()
        st.warning(
            f"شیر بز {selected_goat} تا تاریخ {safe_date} در حالت قرنطینه قرار"
            " گرفت."
        )
